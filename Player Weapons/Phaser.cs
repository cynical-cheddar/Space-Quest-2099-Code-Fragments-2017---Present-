   ...
   float disabledTime;
    ...
    void Update()
    {
        ...
        if (Input.GetMouseButtonDown(0) && ammo > 0 && CF.CanShoot == true)
        {
            ...
            this.GetComponent<PhotonView>().RPC("CreateServerBeam", PhotonTargets.Others);         //RPC
            viewModelAnimator.SetBool("FirePhaser", true);
        }
        if (Input.GetMouseButtonUp(0) || ammo <= 0)
        {
            ...
            viewModelAnimator.SetBool("FirePhaser", false);
        }
        ...
        if (Input.GetMouseButton(0) && ammo > 0 && CF.CanShoot == true)
        {
            ...
            if (cooldown <= 0)
            {
                this.GetComponent<PhotonView>().RPC("LaserPositionServer", PhotonTargets.Others, hit.point);         //RPC
            }
    ...
    [PunRPC]
   public void LaserPositionServer(Vector3 hitPoint) // We need to somehow interpolate the hit position between updates.
    {
    ...
    void OnEnable()
    {
        ...
        float timeDifference = enabledTime - disabledTime;
        ammo += (Mathf.RoundToInt(timeDifference * 10));
        if(ammo > 100)
        {
            ammo = 100;
        }
